...
(
  select *,
    row_number() over(partition by vendorid, tpep_pickup_datetime) as rn -- Deduplication
  from {{ source('staging','yellow_tripdata') }}
  where vendorid is not null 
)
select
   -- identifiers
    {{ dbt_utils.generate_surrogate_key(['vendorid', 'tpep_pickup_datetime']) }} as tripid,    -- Unique surrogate key
